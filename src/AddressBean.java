...
public class AddressBean implements Serializable {
    ...
    public double getAmount() {
        ...
        System.out.println(pizzaQn);

        System.out.println(drinkId);
        System.out.println(drinkQn);

        System.out.println(sidesID);
        System.out.println(sidesQn);
        ...
        if (drinkId == 1) {
            drinkPrice = drinkQn * 2.0;
        }

        if (drinkId == 2) {
            drinkPrice = drinkQn * 1.0;
        }

        // check sides id in order to get correct price per item
        if (sidesID == 1) {
            sidesPrice = sidesQn * 4.0;
        }

        if (sidesID == 2) {
            sidesPrice = sidesQn * 6.0;
